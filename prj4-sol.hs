...
deptEmployees [] _ = []
deptEmployees (x:xs) dpt
  | (emplDept x) == dpt = x :  deptEmployees xs dpt
  | otherwise = deptEmployees xs dpt
...
expn :: (Floating a) => a -> [a]
expnX :: (Fractional a) => a -> a -> a -> a -> [a]
expnX x num deno f = v:expnX x newnum newdeno newf
                        where
                            v = num/deno
                            newnum = num * x
                            newdeno = deno * f
                            newf = f + 1
expn x = expnX x 1.0 1.0 1
...
foldTree node leaf (Leaf v) = leaf v
foldTree node leaf (Tree l v r) = node (foldTree node leaf l) v (foldTree node leaf r)


testFoldTree = do
  assertEq "foldTree (left + v + right) (2*v)"
           (foldTree (\left v right -> left + v + right) (\v -> 2*v)
